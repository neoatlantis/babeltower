	...
	__nest__ (
		...
		'services.auth', {
			__all__: {
				__inited__: false,
				__init__: function (__all__) {
					var __name__ = 'services.auth';
					var GenericInterface = __init__ (__world__.util.interface).GenericInterface;
					...
					'</use>')
					__pragma__ ('<all>')
						__all__.Authentication = Authentication;
						__all__.GenericInterface = GenericInterface;
						__all__.__name__ = __name__;
					__pragma__ ('</all>')
				}
			}
			...
			__all__: {
				...
				__init__: function (__all__) {
					...
					var Event = __class__ ('Event', [object], {
						...
						get trigger () {return __get__ (this, function (self) {
							var args = tuple ([].slice.apply (arguments).slice (1));
							self.__call__ (...args);
						});},
	...
	__nest__ (
		...
		'util.interface', {
			__all__: {
				__inited__: false,
				__init__: function (__all__) {
					var __name__ = 'util.interface';
					var Event = __init__ (__world__.util.event).Event;
					var GenericInterface = __class__ ('GenericInterface', [object], {
						__module__: __name__,
						get event () {return __get__ (this, function (self, py_name) {
							if (!__in__ (py_name, self.events)) {
								var __except0__ = Exception ('Accessing unknown event: {}'.format (py_name));
								__except0__.__cause__ = null;
								throw __except0__;
							}
							return self.events [py_name];
						});},
						get __init__ () {return __get__ (this, function (self, options) {
							var events = getattr (options, 'events', list ([]));
							self.events = dict ({});
